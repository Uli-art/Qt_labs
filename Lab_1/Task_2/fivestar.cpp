...
void FiveStar::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    int a = 0;
    QPoint p[5 * 2 + 1]; //Массив для хранения координат вершин звезды
...
double FiveStar::square(){
    return this->getK() * this->getK() * (3.14  * radius * radius + (3.14  * Radius * Radius - 3.14  * radius * radius) / 3);
}

double FiveStar::perimetr(){
