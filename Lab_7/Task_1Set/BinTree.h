...
public:
    ...
    bool find(KeyType key,const  ValueType& value);

    void traversePreorder(Node* root, std::list<KeyType>& out);
    void traverseInorder(Node* root, std::list<KeyType>& out);
...
template <typename K, typename V>
bool BinTree<K, V>::find(K key, const V& value) {
    if(_root == nullptr) {
        return false;
    }
...
template <typename K, typename V>
void BinTree<K, V>::traverseInorder(Node* root, std::list<KeyType>& out) {
    if(root == nullptr) {
        return;
    }
