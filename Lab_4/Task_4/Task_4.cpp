...
int main()
{
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            ...
            if (up) {
                if ((matr[i][j] >= matr[i - 1][j]) || (matr[i][j] >= matr[i - 1][j - 1]) || (matr[i][j] >= matr[i - 1][j + 1])) continue;
            }
            if (down) {
                if ((matr[i][j] >= matr[i + 1][j]) || (matr[i][j] >= matr[i + 1][j - 1]) || (matr[i][j] >= matr[i + 1][j + 1])) continue;
            }
            if (right) {
                if ((matr[i][j] >= matr[i + 1][j + 1]) || (matr[i][j] >= matr[i][j + 1]) || (matr[i][j] >= matr[i - 1][j + 1])) continue;
            }
            if (left) {
                if ((matr[i][j] >= matr[i + 1][j - 1]) || (matr[i][j] >= matr[i][j - 1]) || (matr[i][j] >= matr[i - 1][j - 1])) continue;
            }
